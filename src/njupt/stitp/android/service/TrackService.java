...
public class TrackService extends Service {
	...
	public void onCreate() {
		...
		username= new SPHelper().getInfo(getApplicationContext(), "userInfo", "username");
	...
	public class MyLocationListener implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (location.getLocType() == BDLocation.TypeOffLineLocation
				...
				track.setAddTime(new Date().toString());
			...
			if(tracks.size()>=72){
				...
				if(unCommitTracks.size()>0){
					if(serverHelper.uploadTrackAndAPP(path, unCommitTracks,null)){
